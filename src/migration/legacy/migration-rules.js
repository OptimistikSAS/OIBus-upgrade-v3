...
import { createConfigDatabase, getConfig, upsertConfig } from './database.service.js'
import { createFolder, filesExists, generateRandomId } from '../../service/utils'
...
export default {
  ...
  30: async (config, logger) => {
    ...
    config.engine.proxies = config.engine.proxies ? config.engine.proxies.map((proxy) => ({ ...proxy, id: generateRandomId() })) : []
  },
  ...
  99: async(config, logger) => {
    logger.info('Config file up to date')
  }
}
